...
fetch(forecast)
    ...
    /*-------SMAL Current Weather Information -------------*/
...
.then((jsObject) => {
    ...
    fivedayforecast.forEach(forecast => {
        ...
        const imagesrc = 'https://openweathermap.org/img/wn/' + forecast.weather[0].icon + '.png';
        const desc = forecast.weather[0].description;
        document.getElementById(`ico${day + 1}`).setAttribute('src', imagesrc);
        document.getElementById(`ico${day + 1}`).setAttribute('alt', desc);
        day++;
    });
});